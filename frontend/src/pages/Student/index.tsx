...
const Student: React.FC = () => {
  ...
  const handleBorrowSubmit = async (values: any) => {
    ...
    try {
      ...
      if (response.ok) {
        message.success('Gửi yêu cầu mượn thành công! Chờ phê duyệt của quản trị viên.');
  ...
  const equipmentColumns = [
    ...
    },
    {
      ...
      dataIndex: ['quantity'],
      key: 'quantity',
      render: (quantity: number, record: Equipment) => {
        const available = quantity - record.borrowed;
        const status =
          available === 0 ? 'error' : available <= 2 ? 'warning' : 'success';
        return <Badge status={status} text={`${available} / ${quantity}`} />;
      },
    },
    {
      title: 'Hành Động',
      ...
      render: (_: any, record: Equipment) => (
        <Button
          ...
          disabled={record.quantity - record.borrowed <= 0}
        >
          Mượn
        </Button>
      ),
    ...
    <Layout style={{ minHeight: '100vh' }}>
      ...
      <Layout>
        <Sider width={200} className="student-sider">
          ...
          >
            <Menu.Item key="equipment" icon={<HomeOutlined />}>
              Danh Sách Thiết Bị
            </Menu.Item>
        ...
        <Layout>
          <Content style={{ padding: '24px' }}>
            {activeTab === 'equipment' && (
              <Card title="Danh Sách Thiết Bị Có Sẵn" className="content-card">
                <Table
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: 'Vui lòng nhập số lượng!' },
              {
                type: 'number',
                max:
                  selectedEquipment && selectedEquipment.quantity - selectedEquipment.borrowed,
